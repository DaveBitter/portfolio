...
const ArticlePage = ({ headings, article, relatedArticles, type }: IProps) => {
  return (
    <>
      <Article {...article} articleType={type} />

      {!!relatedArticles.length && (
        <div className="grid">
          <div className="g2">
            <h2 className="text-colored h1" data-reveal-in-view>
              {type === "friday-tips" ? headings.watchMore : headings.readOn}
            </h2>
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const articleData = articles.find((article: ArticleInterface) =>
    context && context.params ? article.slug === context.params.slug : false
  );

  const ogImage = await getOGImage(`/articles_${articleData.slug}`, {
    title: articleData.title,
    image: articleData.teaserImage,
  ...
  return {
    props: {
      ...
      relatedArticles: articles
        .filter((article: ArticleInterface) =>
          article.slug !== context && context.params
            ? context.params.slug
            : false
        )
        .splice(0, 3),
