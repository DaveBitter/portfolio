...
export default async (path: string) => process.env.CI ? fetchFromLocalMarkdown(path) : await fetch(`${baseUrl}/api${path}`).then((res) => res.json()).catch(() => fetchFromLocalMarkdown(path));
