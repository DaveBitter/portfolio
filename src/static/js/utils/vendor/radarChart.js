    ...
    const wrap = (text, width) => {
        text.each(function () {
            var text = d3.select(this),
                ...
                tspan = text.text(null).append('tspan').attr('x', x).attr('y', y).attr('dy', dy + 'em').style('fill', '#CDCDCD')
                ;
            ...
            while (word = words.pop()) {
                ...
                if (tspan.node().getComputedTextLength() > width) {
                    ...
                    tspan.text(line.join(' '));
                    line = [word];
                    tspan = text.append('tspan').attr('x', x).attr('y', y).attr('dy', ++lineNumber * lineHeight + dy + 'em').style('fill', '#CDCDCD').text(word);
                }
            }
        });
    };//wrap
