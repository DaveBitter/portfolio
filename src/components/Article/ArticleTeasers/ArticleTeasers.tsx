...
// Component
const ArticleTeasers = ({ articles, tags, type }: IProps) => {
  return (
    <>
      {type !== "friday-tips" && tags && !!tags.length && (
        <Tag.Wrapper scrollable={true}>
      ...
      <ul className="article-teasers">
        {articles.map((article: ArticleInterface, index: number) => (
          <li key={index} className="article-teasers__item" data-reveal-in-view>
            <Card>
              <ArticleTeaser
                {...article}
                href={`/${article.type}/[slug]?slug=${article.slug}`}
                as={`/${article.type}/${article.slug}`}
              />
            </Card>
          </li>
        ))}
